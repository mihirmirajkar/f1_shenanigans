...
unsigned long histLapTimes[MAX_LAPS];
int histLapCount = 0;
...
void addLapTime(unsigned long ms) {
  for (int i = MAX_LAPS - 1; i > 0; i--) {
    histLapTimes[i] = histLapTimes[i - 1];
  }
  histLapTimes[0] = ms;
  if (histLapCount < MAX_LAPS) histLapCount++;
}
  ...
  <script>
    ...
    async function fetchStatus() {
      try {
        ...
        serverLapTimes = data.histLapTimes || [];
...
<body>
  ...
  <script>
    ...
    function updateLapTable(data) {
      ...
      if (!tbody) return;
      tbody.innerHTML = '';
      for (let i=0;i<data.histLapCount.length;i++){
        const tr = document.createElement('tr');
        const carNum = i+1;
    ...
    async function fetchRaceStatus() {
      try {
        ...
        if (data.histLapCount && data.lastLapMs && data.bestLapMs) {
          updateLapTable(data);
        }
...
void handleStatus() {
  ...
  json += "\"mode\":\"" + modeStr + "\",";
  json += "\"histLapTimes\":[";
  for (int i = 0; i < histLapCount; i++) {
    json += String(histLapTimes[i]);
    if (i < histLapCount - 1) json += ",";
  }
  json += "]}";
...
void handleRaceStatus() {
  ...
  // Arrays: histLapCount, lastLapMs, bestLapMs
  json += ""histLapCount":[";
